...
function unsafe_view(A::AbstractArray, I::ViewIndex...)
    @_inline_meta
    SubArray(A, I, map(unsafe_length, index_shape(I...)))
end
...
_maybe_reindex{C<:AbstractCartesianIndex}(V, I, ::Tuple{AbstractArray{C}, Vararg{Any}}) =
    (@_inline_meta; SubArray(V, I, map(unsafe_length, index_shape(I...))))
...
function _maybe_reindex(V, I, ::Tuple{})
    ...
    SubArray(V.parent, idxs, map(unsafe_length, index_shape(idxs...)))
end
...
compute_offset1(parent, stride1::Integer, dims::Tuple{Int}, inds::Tuple{Slice}, I::Tuple) = compute_linindex(parent, I) - stride1*first(indices(parent, dims[1]))  # index-preserving case
...
_find_extended_dims(dims, inds, dim, ::ScalarIndex, I...) = _find_extended_dims(dims, inds, dim+1, I...)

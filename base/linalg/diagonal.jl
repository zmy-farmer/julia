...
/{T<:Number}(D::Diagonal, x::T) = Diagonal(D.diag / x)
*(Da::Diagonal, Db::Diagonal) = Diagonal(broadcast(*, Da.diag, Db.diag))
*(D::Diagonal, V::AbstractVector) = broadcast(*, D.diag, V)
...
(\)(D::Diagonal, b::AbstractVector) = broadcast(\, D.diag, b)
(\)(Da::Diagonal, Db::Diagonal) = Diagonal(broadcast(\, Da.diag, Db.diag))
